...
G6 packages the same three-page application as an owner-only private deployment. The current build contains the browser-local deterministic runtime, the 16 frozen G2 IFC files plus the two-file realistic PG-E clinic example, an optional same-origin Worker AI boundary, no source maps, and no hosted key. Privacy, path, media metadata, dependency license, package vulnerability, and no-key failure-closing checks pass. Owner-only Sites version 9 established the clinic-enabled functional preflight and version 10 carried the non-functional evidence tail. Access remains private to one owner. The current local repair candidate has not been deployed and does not authorize public Sites access. See `docs/g6-deployment-architecture.md`, `docs/g6-privacy-license-audit.md`, and `THIRD-PARTY-NOTICES.md`.

PG-B remains the latest closed presentation-readiness checkpoint, with the published Logo v2 replacement independently closed as `PG-B-R1 PASS`. A bounded `PG-B-R2` local repair crops only excess transparent canvas, preserving every visible source pixel, and reduces the README display width from 420 px to 240 px. The resulting local candidate is 951 × 679 RGBA with a 48 px transparent margin and no GIF dependency. Its public GitHub rendering and mapping are deliberately pending collective authorization; the currently published 1024-square v2 evidence remains historical through final public head `b1727db600ae169eb603c5dd6863b417faef1b9f`. PG-C remains closed; Sites remains owner-only and private.

PG-E is `IN_PROGRESS / LOCAL_REPAIR_PASS_EXTERNAL_SYNC_PENDING_USER_HOSTED_UAT_PENDING`. The user accepted the visual realism of the rebuilt 12 m × 8 m, one-storey community clinic: perimeter and door-gapped internal walls, a ground slab, six columns, three beams, an A–C / 1–4 grid, and room-serving ceiling services. The pair contains seven represented pipe routes plus one deliberately geometry-free imported `IfcPipeSegment` used only to prove failure closing. Against eight walls and three beams they form 88 candidate records: 77 receive an evaluated outcome and 11 fail closed as `NOT_EVALUATED`. Six independently authored sentinels retain 6/6 repeated technical agreement. The prior public/Sites run produced 4 CLASH, 1 WARNING, 11 NOT_EVALUATED, and 72 CLEAR records with working 3D evidence and no console warnings or errors. A structured UAT record now prevents a status-only PASS, but the repair candidate still requires collective publication authorization and the user's final hosted trial. VG, G7A, G7B, and G7 have not started. See `docs/pg-e-engineering-uat.md`.
...
The test regenerates the CC0 pair twice in ignored isolated roots, checks byte-for-byte identity, validates the realistic one-storey planning context, and emits all 88 candidate records through the shipped browser core: 77 evaluated outcomes and 11 deliberate failure-closed records. It also validates the structured pending UAT record. A local technical pass does not substitute for the user Chrome UAT checklist in `docs/pg-e-engineering-uat.md` and performs no GitHub, Sites, permission, key, provider, VG/G7, mobile, or video action.
