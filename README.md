...
G6 packages the same three-page application as a private deployment candidate. The build contains the browser-local deterministic runtime, the 16 public generated IFC files, an optional same-origin Worker AI boundary, no source maps, and no hosted key. Privacy, path, media metadata, dependency license, package vulnerability, and no-key failure-closing checks pass locally. Public access remains unapproved pending the private-preview review and O-006 decision. See `docs/g6-deployment-architecture.md`, `docs/g6-privacy-license-audit.md`, and `THIRD-PARTY-NOTICES.md`.
