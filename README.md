...
G6 packages the same three-page application as an owner-only private deployment. The current build contains the browser-local deterministic runtime, the 16 frozen G2 IFC files plus the two-file realistic PG-E clinic example, an optional same-origin Worker AI boundary, no source maps, and no hosted key. Privacy, path, media metadata, dependency license, package vulnerability, and no-key failure-closing checks pass. Owner-only Sites version 9 established the clinic-enabled hosted preflight; access remains private to one owner, and a later evidence-tail deployment does not authorize public Sites access. See `docs/g6-deployment-architecture.md`, `docs/g6-privacy-license-audit.md`, and `THIRD-PARTY-NOTICES.md`.

PG-B remains the latest closed presentation-readiness checkpoint, with the Logo v2 replacement independently closed as `PG-B-R1 PASS`. The user-selected 1024 × 1024 transparent static Logo v2 now replaces the canonical public README image, displayed at 420 px with updated descriptive alt text, metadata/weight checks, and no GIF dependency. Public GitHub verification found natural size 1024 × 1024, 420 × 420 desktop rendering, approximately 279.33 × 279.33 rendering at a 360 px viewport, no horizontal overflow, and zero console warnings or errors. The 29-path publication mapping matches the local candidate byte for byte through technical-chain tip `b3b03046982011567f1108967dd84672cfad69bd`; v1 evidence remains historical only. PG-C remains closed; Sites remains owner-only and private.

PG-E is `IN_PROGRESS / EXTERNAL_SYNC_PASS_USER_HOSTED_UAT_PENDING`. The user accepted the visual realism of the rebuilt 12 m × 8 m, one-storey community clinic: perimeter and door-gapped internal walls, a ground slab, six columns, three beams, an A–C / 1–4 grid, and room-serving ceiling pipe routes. Eight `IfcPipeSegment` elements are checked against eight walls and three beams, forming 88 candidate pairs and six independently authored sentinels with 6/6 repeated technical agreement. The public website and owner-only Sites version 9 expose this pair as `PG-E · Realistic one-storey clinic · 88 pairs`; an online Chrome run produced 4 CLASH, 1 WARNING, 11 NOT_EVALUATED, and 72 CLEAR records with working 3D evidence and no console warnings or errors. The final user trial on the hosted candidate remains open, so PG-E and overall PG are not yet `PASS`. VG, G7A, G7B, and G7 have not started. See `docs/pg-e-engineering-uat.md`.
...
The published asset is the exact user-selected Logo v2: a 1024 × 1024 RGBA PNG copied byte-for-byte and displayed at a restrained 420 px README width. It keeps its transparent background, declares descriptive alt text, and has no animation, GIF, reduced-motion source, generated layers, or motion-build dependency. The test performs no GitHub or Sites write; the completed Chrome verification and 29/29 remote mapping are recorded in `docs/pg-b-github-logo.md`.
