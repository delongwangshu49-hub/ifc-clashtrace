...
公开 Sites 已上线。可选 Groq 解读位于服务器端，必须逐次预览字段并重新同意，而且不能修改任何确定性记录。

> **网络访问提示：** 已收到中国大陆网络无法直接访问本站的反馈。相关观察的适用范围及本地运行替代方式，见[网络访问说明](#网络访问说明)。
...
更新于 2026 年 9 月 19 日。站点设为公开，并不代表所有网络环境均可访问。

- **中国大陆：** 根据项目维护者反馈，本站在其使用的中国大陆网络环境下无法直接访问。这是已反馈的访问情况，并非对所有地区和运营商的全面测试。
- **中国香港：** 2026 年 9 月 1 日的测试发现，原部署地址在当时测试的香港本地网络下无法直接访问；迁移后的新地址尚未在该网络环境中复测。

原部署曾在上述测试中通过 VPN、日本或新加坡节点及全局路由成功访问。这是历史结果，不保证当前仍然可用。使用第三方网络服务须遵守所在地法律法规、服务条款及所在组织的网络政策；本项目不提供此类服务或配置支持。

如果在线站点无法打开，可在获取源码并安装依赖后，参照[快速开始](#快速开始)在本地运行确定性功能。阅读仓库不依赖在线站点，但 GitHub 和依赖下载本身也有网络要求；可选 AI 还需要另行配置可连接提供方的服务器端路由。
...
上述命令启动静态应用，不会启动 AI API 路由。确定性检查无需 API Key；如需本地 AI，请阅读 [.env.example](.env.example) 与 [AI 架构](docs/g4ai-architecture.md)，另行配置 `npm run g4ai:serve` 路由。`GROQ_API_KEY` 必须保留在服务器端，禁止提交到仓库。
