...
| S-02 | PG-C closed the content audit; PG-B and the published Logo v2 revision remain verified. PG-B-R2 is a local-only transparent-canvas/scale repair awaiting external verification. | Master plan PG-C/PG-B; asset manifest; historical public GitHub checks; local crop contract | PG-C, PG-B, and PG-B-R1 `PASS`; PG-B-R2 `LOCAL_REPAIR_CANDIDATE_EXTERNAL_SYNC_PENDING` |
...
| S-04 | PG-B remains the latest closed checkpoint. PG-E is in progress with accepted visual realism and 6/6 technical sentinel preflight. The prior 29-path release reached public GitHub and owner-only Sites versions 9/10; the current 30-path repair candidate and structured UAT remain unpublished, and the final user hosted trial remains open. VG, G7A, G7B, and G7 have not started. | Master-plan D-071–D-074; `docs/pg-e-engineering-uat.md`; `data/pg-e-uat-record.json` | PG-E `IN_PROGRESS / LOCAL_REPAIR_PASS_EXTERNAL_SYNC_PENDING_USER_HOSTED_UAT_PENDING`; later stages `PLANNED` / not started only |
...
| Development log | Closed timeline through PG-B with Logo v2 public verification recorded; PG-E shown as `IN_PROGRESS` with external synchronization complete and final hosted UAT pending; VG and G7 stages remain `PLANNED`; metrics use C-04/C-09/C-10; Sites privacy uses S-03. |
| README | Closed evidence through PG-B-R1 is retained; Logo v2 public verification and the PG-E selectable hosted example are explicit; final user hosted UAT remains pending; the static PNG has no GIF dependency. |
...
3. PG-B without `data-claim-state="closed"`, `PASS`, and explicit Logo v2 public verification; PG-E without `data-claim-state="in_progress"`, `IN PROGRESS`, accepted visual realism, a selectable one-storey clinic example, 6/6 technical evidence, completed online mapping, and explicit final hosted-UAT pending status; or VG/G7 stages without `planned`/`PLANNED` and matched “尚未开始” / “Not started” wording;
...
The user accepted the clinic's visual effect and previously authorized the exact 29-path release, whose signed-in Chrome GitHub chain, 29/29 byte mapping, owner-only Sites versions 9/10, public Logo checks, and automated hosted clinic run remain historical evidence. The later audit found display-scale, wording, structured-UAT, version-traceability, and live-mapping-guard gaps. The user authorized one bounded local repair and required work to stop before public release. The resulting 30-path repair candidate has no current GitHub/Sites authorization. No public Sites access, access-policy/key change, VG, G7, mobile computation, or video production is authorized or performed.
